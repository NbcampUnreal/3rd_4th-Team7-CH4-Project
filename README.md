...
**네트워크 핵심기능**
> AWS Dedicated Server를 사용하여 NetConnection 기반 PlayerUniqueID를 부여하여 중복 접속을 방지하고 SeamlessTravel을 관리하고 있음
- Nickname 정보 저장과 자동매칭
- 인원 수 미달 시 일정 시간 대기 후 매칭 취소
- 플레이 레벨 일정시간 미접속 시 게임 종료

**OnlineSubsystem 사용**
- Host: UTHGameInstance::HostSession() → GameSession 생성
- Join: JoinServer() → OnFindSessionsComplete 처리 → 무작위 세션 참가
- 클라 이동: GetResolvedConnectString → ClientTravel
