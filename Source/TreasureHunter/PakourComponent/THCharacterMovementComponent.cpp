...
void UTHCharacterMovementComponent::BeginPlay()
{
    ...
    {
        OwningPlayerAnimInstance->OnMontageEnded.AddDynamic(this, &UTHCharacterMovementComponent::OnClimbMontageEnded);
        OwningPlayerAnimInstance->OnMontageBlendingOut.AddDynamic(this, &UTHCharacterMovementComponent::OnClimbMontageEnded);
    }

    OwningPlayerCharacter = Cast<ATHPlayerCharacter>(CharacterOwner);
}

void UTHCharacterMovementComponent::TickComponent(float DeltaTime, enum ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
...
void UTHCharacterMovementComponent::PhysCustom(float deltaTime, int32 Iterations)
{
    if (IsClimbing())
    {
...
float UTHCharacterMovementComponent::GetMaxAcceleration() const
{
    if (IsClimbing())
    {
...
FVector UTHCharacterMovementComponent::ConstrainAnimRootMotionVelocity(const FVector& RootMotionVelocity, const FVector& CurrentVelocity) const
{
...
TArray<FHitResult> UTHCharacterMovementComponent::DoCapsuleTraceMultiByObject(const FVector& Start, const FVector& End, bool bShowDebugShape, bool bDrawPersistentShapes) const
{
...
FHitResult UTHCharacterMovementComponent::DoLineTraceSingleByObject(const FVector& Start, const FVector& End, bool bShowDebugShape, bool bDrawPersistentShapes) const
{
    FHitResult OutHit;
...
bool UTHCharacterMovementComponent::CanStartMantle(FVector& OutStart, FVector& OutEnd) const
{
    ...
    // 1) 가슴~머리 높이에서 전방 표면 찾기
    const float ChestOffset = CharacterOwner->BaseEyeHeight * 0.3f;   // 가슴
    const float HeadOffset = CharacterOwner->BaseEyeHeight * 0.9f;   // 머리 근처

    FHitResult SurfaceHit;
    {
        const FVector Start = CompLoc + Up * ChestOffset;
        const FVector End = Start + Fwd * MantleForwardDistance;
        SurfaceHit = DoLineTraceSingleByObject(Start, End, /*debug*/false);
        if (!SurfaceHit.bBlockingHit)
        {
            const FVector Start2 = CompLoc + Up * HeadOffset;
            const FVector End2 = Start2 + Fwd * MantleForwardDistance;
            SurfaceHit = DoLineTraceSingleByObject(Start2, End2, /*debug*/false);
            if (!SurfaceHit.bBlockingHit) return false;
        }
    }

    // 표면 경사 체크 (너무 가파르면 맨틀)
    const float UpDot = FVector::DotProduct(SurfaceHit.ImpactNormal, FVector::UpVector);
    const float Deg = FMath::RadiansToDegrees(FMath::Acos(FMath::Clamp(UpDot, -1.f, 1.f)));
    // Deg가 0이면 완전 위쪽을 향함(바닥), 90에 가까울수록 수직벽
    // "경사면에서 맨틀"이므로 벽(>80~90)은 제외하고, 너무 평평(바닥)도 제외
    const float MinAngle = 20.f; // 너무 평평하면 맨틀 아님
    if (Deg < MinAngle || Deg > MaxMantleSurfaceAngle)
    {
        // 벽(수직)인 경우는 기존 등반/탑오버 로직으로
        return false;
    }

    // 2) 표면 위쪽 탑면(걷기 가능한 곳) 찾기
    // 충돌 지점에서 약간 위로 올려, 전방으로 오프셋 후 아래로 다운캐스트
    const FVector ProbeStart = SurfaceHit.ImpactPoint + Up * 40.f + Fwd * 30.f;
    const FVector ProbeEnd = ProbeStart + Down * MantleTopDownCheck;

    FHitResult TopHit = DoLineTraceSingleByObject(ProbeStart, ProbeEnd, /*debug*/false);
    if (!TopHit.bBlockingHit) return false;

    // 탑면이 어느정도 수평인지 확인 (걸을만한지)
    const float TopDot = FVector::DotProduct(TopHit.ImpactNormal, FVector::UpVector);
    if (TopDot < KINDA_SMALL_NUMBER) return false;

    // 워핑 타겟 세팅용 반환
    OutStart = SurfaceHit.ImpactPoint;
    // 발 딛는 곳에 캡슐 반지름만큼 살짝 앞으로 빼주면 클립 방지
    const float CapsuleRadius = CharacterOwner->GetCapsuleComponent()->GetScaledCapsuleRadius();
    OutEnd = TopHit.ImpactPoint + TopHit.ImpactNormal * 2.f + Fwd * CapsuleRadius;

    return true;
}
void UTHCharacterMovementComponent::ToggleClimbing(bool bAttemptClimbing)
{
    if (bAttemptClimbing)
    {
        if (CanStartClimbing())
...
void UTHCharacterMovementComponent::TryStartVaulting()
{
...
bool UTHCharacterMovementComponent::CanStartVaulting(FVector& OutVaultStartPosition, FVector& OutVaultLandPosition)
{
    if (IsFalling())
        return false;
...
bool UTHCharacterMovementComponent::CanClimbDownLedge()
{
    if (IsFalling())
        return false;
...
bool UTHCharacterMovementComponent::CanStartClimbing()
{
    if (IsFalling())
        return false;
...
void UTHCharacterMovementComponent::StartClimbing()
{
    SetMovementMode(MOVE_Custom, ECustomMovementMode::MOVE_Climb);
}

void UTHCharacterMovementComponent::StopClimbing()
{
    if (IsClimbing())
    {
...
bool UTHCharacterMovementComponent::CheckHasReachedLedge()
{
...
bool UTHCharacterMovementComponent::IsClimbing() const
{
    return MovementMode == MOVE_Custom && CustomMovementMode == ECustomMovementMode::MOVE_Climb;
}

void UTHCharacterMovementComponent::PhysClimb(float deltaTime, int32 Iterations)
{
...
void UTHCharacterMovementComponent::ProcessClimbableSurfaceInfo()
{
...
bool UTHCharacterMovementComponent::ShouldStopClimbing()
{
    if (ClimbableSurfacesTracedResults.IsEmpty())
        return true;
...
bool UTHCharacterMovementComponent::CheckHasReachedFloor()
{
    const FVector DownVector = -UpdatedComponent->GetUpVector();
...
FQuat UTHCharacterMovementComponent::GetClimbRotation(float DeltaTime)
{
...
void UTHCharacterMovementComponent::SnapMovementToClimbableSurfaces(float DeltaTime)
{
    const FVector ComponentForward = UpdatedComponent->GetForwardVector();
    const FVector ComponentLocation = UpdatedComponent->GetComponentLocation();
...
TArray<FHitResult> UTHCharacterMovementComponent::GetClimbableSurfaces()
{
...
FHitResult UTHCharacterMovementComponent::TraceFromEyeHeight(float TraceDistance, float TraceStartOffset, bool bShowDebugShape, bool bDrawPersistantShapes)
{
    const FVector ComponentLocation = UpdatedComponent->GetComponentLocation();
...
void UTHCharacterMovementComponent::PlayClimbMontage(UAnimMontage* MontageToPlay)
{
    if (!MontageToPlay)
        return;
    if (!OwningPlayerAnimInstance)
...
void UTHCharacterMovementComponent::OnClimbMontageEnded(UAnimMontage* Montage, bool bInterrupted)
{
...
void UTHCharacterMovementComponent::RequestHopping()
{
...
void UTHCharacterMovementComponent::SetMotionWarpTarget(const FName& InWarpTargetName, const FVector& InTargetPosition)
{
...
void UTHCharacterMovementComponent::HandleHopUp()
{
...
bool UTHCharacterMovementComponent::CheckCanHopUp(FVector& OutHopUpTargetPosition)
{
...
void UTHCharacterMovementComponent::HandleHopDown()
{
...
bool UTHCharacterMovementComponent::CheckCanHopDown(FVector& OutHopDownTargetPosition)
{
...
FVector UTHCharacterMovementComponent::GetUnrotatedClimbVelocity() const
{
